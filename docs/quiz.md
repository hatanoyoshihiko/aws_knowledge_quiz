...
このドキュメントでは、AWSクイズアプリケーションのクイズ生成メカニズムを詳しく説明します。

## 概要

クイズ生成は以下の3つの主要コンポーネントで構成されています：
...
QUERY_COMPONENTSで定義された要素から、決定的な組み合わせを生成します。
...
QUESTION_STYLESが6種類あるため、理論上は以下の通りです：
...
**計算の意味**:
- 同じMCP検索クエリでも、出題スタイルが異なれば異なる問題になります
...
**注意**: 11,460通りは理論上の最大値です。実際には以下の要因により、生成されるクイズの種類はさらに多様になります：
...
Host UIで「次のクイズ」ボタンを押すと、以下の処理が実行されます：
...
Host UIとTeam UIは15秒ごとにポーリングして最新クイズを取得します：
