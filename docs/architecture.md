...
このドキュメントでは、AWSクイズアプリケーションのシステムアーキテクチャを図で説明します。

> **注意**: 本図は処理全体の俯瞰を目的としており、クイズ状態・スコアの同期（GET /quiz/current, /scores）はポーリングとして簡略表現しています。詳細な処理順は各シーケンス図を参照してください。
...
詳細は [クイズ生成の仕組み](./quiz.md#重複回避の仕組み) を参照してください。
