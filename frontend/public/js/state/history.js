...
function saveHistory() {
  localStorage.setItem(
    HISTORY_KEY,
    JSON.stringify(state.history.slice(0, HISTORY_MAX))
  );
}
...
export function renderHistory() {
  ...
  for (const h of state.history) {
    ...
    btn.className =
      "w-full text-left rounded-2xl border border-slate-800 bg-slate-950 p-3 " +
      "hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-300";
    ...
    btn.innerHTML = `
      <div class="flex items-start justify-between gap-2">
        ...
        <span class="shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ${pillClass}">
          ${escapeHtml(r)}
        </span>
      </div>
    `;
    ...
    btn.addEventListener("click", () => {
      ...
      // history.js は「復習要求イベント」を投げるだけ
      window.dispatchEvent(
        new CustomEvent("quiz:review", { detail: { questionId: h.questionId } })
      );

      toast("復習モードに切り替えました（更新で最新に戻ります）");
      el("answer")?.focus();
    });
