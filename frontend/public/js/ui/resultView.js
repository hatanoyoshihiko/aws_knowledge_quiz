...
export function resetResult() {
    ...
    // Reset example answer section
    const exampleSection = el("exampleAnswerSection");
    console.log("[DEBUG] resetResult: exampleSection =", exampleSection);

    if (exampleSection) {
        console.log("[DEBUG] resetResult: adding 'hidden' to exampleSection");
        exampleSection.classList.add("hidden");
    }
    const exampleWrap = el("exampleAnswerWrap");
    if (exampleWrap) {
        exampleWrap.classList.add("hidden");
    }
    const exampleText = el("exampleAnswerText");
    if (exampleText) {
...
function toScore100(score01) { // score01: 0.0〜1.0 を想定（念のため clamp）
...
export function setResult(result) {
    ...
    const met = Array.isArray(result.mustPointsMetDetails) && result.mustPointsMetDetails.length ? result.mustPointsMetDetails.map(formatDetail) : (Array.isArray(result.mustPointsMet) ? result.mustPointsMet.map(normalizeP) : []);

    const missing = Array.isArray(result.missingMustPointsDetails) && result.missingMustPointsDetails.length ? result.missingMustPointsDetails.map(formatDetail) : (Array.isArray(result.missingMustPoints) ? result.missingMustPoints.map(normalizeP) : []);
    ...
    if (met.length || missing.length || hint) {
        ...
        el("metPoints").innerHTML = met.map(x => `<li>${
            escapeHtml(x)
        }</li>`).join("");
        el("missingPoints").innerHTML = missing.map(x => `<li>${
            escapeHtml(x)
        }</li>`).join("");
    ...
    // Show example answer section after scoring
    const exampleSection = el("exampleAnswerSection");
    console.log("[DEBUG] setResult: exampleSection =", exampleSection);
    console.log("[DEBUG] setResult: exampleSection exists?", !! exampleSection);

    if (exampleSection) {
        console.log("[DEBUG] setResult: BEFORE remove - classList =", exampleSection.classList.toString());
        console.log("[DEBUG] setResult: BEFORE remove - className =", exampleSection.className);
        exampleSection.classList.remove("hidden");
        console.log("[DEBUG] setResult: AFTER remove - classList =", exampleSection.classList.toString());
        console.log("[DEBUG] setResult: AFTER remove - className =", exampleSection.className);
        console.log("[DEBUG] setResult: computed display =", window.getComputedStyle(exampleSection).display);
    } else {
        console.error("[DEBUG] setResult: exampleAnswerSection element NOT FOUND!");
    }
}
